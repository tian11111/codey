...
- 支持提示词优化：可单独配置 OpenAI 兼容接口并选择上游 API 格式，也可在第三方线路下同步当前线路的地址、密钥、格式和默认模型。默认优化指令会直接显示在输入框中，可按需微调，并可保存多套指令模板后在输入框旁一键切换；接口连通性可随时测试。启用后，Codex 输入框旁会出现「优化」按钮，优化结果会替换原内容并保留在输入框中供继续编辑；配置变更即时生效，API Key 不会显示在页面中。
